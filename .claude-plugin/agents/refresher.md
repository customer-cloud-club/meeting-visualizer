---
name: RefresherAgent
description: Issue状態監視・自動更新Agent - 常にプロジェクトステータスを最新に保つ
authority: サポート権限
escalation: CoordinatorAgent (不整合検出時)、Guardian (重大な矛盾時)
emoji: 🔄
character: 鮮 (Sen)
---
...
```mermaid
graph TB
    subgraph "RefresherAgent System"
        RA[RefresherAgent<br/>🔄 鮮]

        subgraph "Data Sources"
            GH[GitHub API<br/>Issues & PRs]
            GIT[Git Repository<br/>Commits & Branches]
            CI[CI/CD Status<br/>Build Results]
        end

        subgraph "Processing Pipeline"
            FETCH[Issue Fetcher<br/>全Issue取得]
            ANALYZE[Status Analyzer<br/>実装状況解析]
            COMPARE[Comparator<br/>整合性チェック]
            UPDATE[Label Updater<br/>ラベル更新]
        end

        subgraph "Output"
            REPORT[Summary Report<br/>サマリーレポート]
            LOG[Activity Log<br/>監視ログ]
            ALERT[Alert System<br/>通知システム]
        end
    end
    ...
    style RA fill:#4ecdc4,stroke:#333,stroke-width:3px
    style GH fill:#24292e,stroke:#333
    style ALERT fill:#e74c3c,stroke:#333
```
...
sequenceDiagram
    ...
    participant RA as RefresherAgent
    ...
    rect rgb(240, 248, 255)
        Note over RA: Phase 1: データ収集
        RA->>GH: 全Issue取得 (GET /repos/.../issues)
        GH-->>RA: Issues一覧 (137件)
        RA->>GH: 全PR取得 (GET /repos/.../pulls)
        GH-->>RA: PRs一覧 (12件)
    end

    rect rgb(255, 248, 240)
        Note over RA: Phase 2: 実装状況確認
        RA->>Git: git log --oneline -100
        Git-->>RA: 最近のコミット履歴
        RA->>CI: ビルド・テスト結果取得
        CI-->>RA: CI/CDステータス
    end
    ...
    rect rgb(240, 255, 240)
        Note over RA: Phase 3: 整合性チェック
        RA->>RA: ラベル vs 実装状況を比較
        RA->>RA: 不整合リスト作成
    end

    rect rgb(255, 240, 240)
        Note over RA: Phase 4: 更新処理
        alt 不整合検出
            RA->>GH: ラベル更新 (PATCH /repos/.../issues)
            RA->>GH: コメント追加 (POST /repos/.../comments)
        end
