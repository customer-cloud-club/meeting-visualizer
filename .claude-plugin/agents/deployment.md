...
```mermaid
flowchart TD
    A[Deploy Command] --> B{Has DATABASE_URL in app?}
    B -->|No| C[Skip DB setup]
    B -->|Yes| D[Read .ccagi.yml]
    D --> E{Environment?}
    E -->|development| F[Use Sidecar Strategy]
    E -->|production| G[Use RDS Strategy]

    F --> F1[Add postgres container]
    F --> F2[Set localhost connection]
    F --> F3[Add container dependency]

    G --> G1{RDS exists?}
    G1 -->|No| G2[Create RDS instance]
    G1 -->|Yes| G3[Get RDS endpoint]
    G2 --> G4[Store creds in Secrets Manager]
    G3 --> G5[Set DATABASE_URL from endpoint]
    G4 --> G5
```
