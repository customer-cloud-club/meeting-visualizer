...
| **ccagi-orchestrator** | Rust API | Coordination |
...
```rust
// crates/ccagi-aws-agent/src/types.rs
...
impl AWSAgent {
    async fn register_service_agent(&self, resource: &AwsResource) -> Result<ServiceAgent> {
        ...
        // Store in ccagi-composite-state
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": "arn:aws:iam::*:role/CcagiAgentRole"
    }
  ]
}
```
...
4. **Resource Tagging**: Tag all resources with `CcagiManaged=true`
...
# Ccagi crates
ccagi-types = { path = "../ccagi-types" }
ccagi-agent-core = { path = "../ccagi-agent-core" }
ccagi-composite-state = { path = "../ccagi-composite-state" }
...
**Contact**: Through Ccagi framework
