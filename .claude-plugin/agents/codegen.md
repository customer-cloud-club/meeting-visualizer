...
### 既存コードポリシー (Existing Code Policy)

`.ccagi.yml` の `agents.codegen_agent.existing_code_policy` に従って既存コードを扱います。

```yaml
# .ccagi.yml から参照
existing_code_policy:
  new_files: create           # 新規ファイルは作成
  existing_files: patch       # 既存ファイルは差分適用
  protected_patterns:         # 保護対象は変更禁止
    - "*.config.*"
    - "package.json"
  major_changes: require_review  # 大幅変更時はレビュー必須
  conflict_resolution: prompt    # コンフリクト時は確認
  backup_before_modify: true     # 変更前にバックアップ
```
...
│ Step 3: 変更量チェック               │
├─────────────────────────────────────┤
│ major_change_threshold 超過         │
│   → major_changes ポリシー適用       │
│ 超過しない → existing_files 適用     │
└─────────────────────────────────────┘
        ↓
┌─────────────────────────────────────┐
│ Step 4: バックアップ                 │
...
│ Step 5: 変更適用                     │
├─────────────────────────────────────┤
│ patch → Edit tool で差分適用         │
│ overwrite → Write tool で上書き      │
│ skip → 変更なし                      │
└─────────────────────────────────────┘
```
...
```typescript
async function applyCodeGenPolicy(
  filePath: string,
  generatedCode: string,
  policy: ExistingCodePolicy
): Promise<ApplyResult> {
  const fileExists = await fs.exists(filePath);

  // Step 1: 新規ファイル
  if (!fileExists) {
    return { action: 'create', file: filePath };
  }

  // Step 2: 保護パターンチェック
  if (matchesProtectedPattern(filePath, policy.protected_patterns)) {
    return { action: 'skip', reason: 'protected', file: filePath };
  }

  // Step 3: 変更量チェック
  const existingCode = await fs.readFile(filePath, 'utf-8');
  const diff = calculateDiff(existingCode, generatedCode);

  if (diff.changedLines > policy.major_change_threshold) {
    if (policy.major_changes === 'require_review') {
      return { action: 'pending_review', diff, file: filePath };
    } else if (policy.major_changes === 'reject') {
      return { action: 'skip', reason: 'major_change', file: filePath };
    }
  }

  // Step 4: バックアップ
  if (policy.backup_before_modify) {
    await backupFile(filePath, policy.backup_directory);
  }

  // Step 5: 変更適用
  switch (policy.existing_files) {
    case 'patch':
      return { action: 'patch', diff, file: filePath };
    case 'overwrite':
      return { action: 'overwrite', file: filePath };
    case 'skip':
      return { action: 'skip', reason: 'policy', file: filePath };
  }
}
```
