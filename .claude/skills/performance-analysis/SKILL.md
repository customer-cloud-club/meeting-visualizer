...
```bash
# フレームグラフ生成
cargo flamegraph --bin ccagi -- --issue 270
...
```bash
# valgrind massif
valgrind --tool=massif ./target/release/ccagi
ms_print massif.out.*

# heaptrack（推奨）
heaptrack ./target/release/ccagi
heaptrack_gui heaptrack.ccagi.*
```
...
# Step 2: プロファイル収集
./target/release/ccagi [typical workload]
