---
description: CCAGI Water Spider全自動モード起動
---

# CCAGI Water Spider全自動モード
...
最大10 Issue、30秒間隔:
ccagi__auto({ maxIssues: 10, interval: 30 })
```
...
# 最大Issue数と間隔を指定
npx ccagi auto --max-issues 10 --interval 30

# 1回だけ実行（ポーリングなし）
npx ccagi auto --max-issues 5 --interval 0
```
...
```bash
# maxIssues到達で自動停止
npx ccagi auto --max-issues 3

# interval=0 で1回のみ実行
npx ccagi auto --interval 0
```
...
Claude: [ccagi__auto({ maxIssues: 5 }) を実行]
...
Claude: [ccagi__auto({ maxIssues: 10, interval: 30 }) を実行]
...
**通常運用**:
```bash
npx ccagi auto --max-issues 10 --interval 60
```

**夜間バッチ処理**:
```bash
npx ccagi auto --max-issues 50 --interval 0
```

**テスト・検証**:
```bash
npx ccagi auto --max-issues 1 --interval 0 --dry-run
```
