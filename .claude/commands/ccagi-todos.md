...
```
TODOをスキャンして表示:
ccagi__todos({ path: "./src" })

TODOを検出して自動的にIssue作成:
ccagi__todos({ path: "./src", autoCreate: true })
...
```bash
# TODOをスキャンして表示
npx ccagi todos --path ./src

# 自動的にIssue作成
npx ccagi todos --path ./src --auto-create

# 特定のパターンのみ検出
npx ccagi todos --path . --pattern "TODO|FIXME"

# ドライラン（Issue作成しない）
npx ccagi todos --path ./src --auto-create --dry-run
```
...
推奨アクション:
  1. npx ccagi todos --auto-create でIssue化
  2. 高優先度(P0/P1)から対応
```
...
Generated by CCAGI Todos Auto Mode
...
Claude: [ccagi__todos({ path: "./src" }) を実行]
...
Claude: [ccagi__todos({ path: ".", autoCreate: true }) を実行]
